...
class StarCoderInfill(HFTorchDecoder):
    ...
    def codegen(
        ...
        #transition_scores = self.model.compute_transition_scores(
            #raw_outputs.sequences, raw_outputs.scores, normalize_logits=True
            #)
        #print(raw_outputs)
        #print(transition_scores)
        #gen_scores = transition_scores[:, len(input_tokens[0]) :]
        gen_strs = self.tokenizer.batch_decode(
            gen_seqs, skip_special_tokens=self.skip_special_tokens
        )
        outputs = []
        ...
        for output in gen_strs:
            ...
            for eos in self.eos:
                if eos in output:
                    min_index = min(min_index, output.index(eos))
            outputs.append(output[:min_index])
        ### code for getting the output_scores as well ###
        
        # find if one list is in another list
        #def find_sublist_index(sublist, mainlist):
            #"""Find if sublist is in mainlist and return the index of the first element of sublist in mainlist."""
            #for i in range(len(mainlist) - len(sublist) + 1):
                #if mainlist[i:i+len(sublist)] == sublist:
                    #return i
            #return -1  # Return -1 if sublist is not found in mainlist
    
        # removes eos tokens from output_scores
        #for i, gen_seq in enumerate(gen_seqs):
            #min_index = 10000
            #for eos in self.eos:
                #eos_idx = find_sublist_index(self.tokenizer(eos)["input_ids"], gen_seq.tolist())
                #if eos_idx != -1:
                    #min_index = min(min_index, eos_idx)
            #print(gen_scores[i][:min_index])
            #output_scores.append(gen_scores[i][:min_index].to(torch.float32).cpu().numpy().tolist())
        ### end of code ###
        return outputs
